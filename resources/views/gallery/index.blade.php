...
<body>
    ...
    <a href="{{ route('gallery.create') }}" class="upload-link"><i class="fas fa-upload"></i> Upload New Image</a>
    <a href="{{ route('gallery.myMedia') }}" class="my-media-link"><i class="fas fa-user"></i> My Media</a>
    @endif

    <!-- Category Buttons Section -->
    <div class="category-buttons">
        @if (auth()->guard('staff')->check())
        <a class="category-button" href="{{route('category.create')}}">new category</a>
        @endif
        ...
        <form action="{{ route('category.destroy', $category->id) }}" method="POST" style="margin-left: 5px;">
            @csrf
            @method('DELETE')
            <button type="submit" class="button delete" onclick="return confirm('Are you sure you want to delete this category?');">
    ...
    @if (Route::currentRouteName() === 'gallery.myMedia')
    <a href="{{ route('gallery.index') }}" class="back-button"><i class="fas fa-arrow-left"></i> Back to Gallery</a>
    @endif
    ...
    <div class="gallery-grid" id="gallery">
        ...
        <div class="gallery-item" data-category-id="{{ $image->category_id }}">
            ...
            <span>
                <a href="{{ route('gallery.edit', $image->id) }}" class="button">Edit</a>
                <form action="{{ route('gallery.destroy', $image->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="button delete" onclick="return confirm('Are you sure you want to delete this image?')">Delete</button>
                </form>
            </span>
